...
using static Denpou.Base.Async;

namespace Denpou.Controls.Hybrid;
...
public class TaggedButtonGrid : MultiView
{
    ...
    private int LayoutRows
    {
        get
        {
            ...
            if (EnableCheckAllTools && SelectedViewIndex == 1) layoutRows++;
        ...
        switch (SelectedViewIndex)
        {
            case 0:
                ...
                else if (EnableSearch)
                {
                    if (result.MessageText.StartsWith("🔍"))
                    {
                        ...
                        if (SearchQuery == null) await Device.Send(SearchLabel);
            ...
            case 1:
                ...
                if (result.MessageText == CheckAllLabel)
                    CheckAllTags();
                else if (result.MessageText == UncheckAllLabel) UncheckAllTags();
    ...
    private async Task RenderTagView()
    {
        ...
        foreach (var t in Tags)
        {
            ...
            if (SelectedTags?.Contains(t) ?? false) s += " ✅";
    ...
    private async Task RenderDataView()
    {
        ...
        if (Tags != null && SelectedTags != null) form = form.TagDuplicate(SelectedTags);

        if (EnablePaging) IntegratePagingView(form);
    ...
    {
        //No Items 
        if (dataForm.Rows == 0) dataForm.AddButtonRow(new ButtonBase(NoItemsLabel, "$"));
        ...
        if (IsNavigationBarVisible)
        {
            ...
            if (Tags != null && Tags.Count > 0) row.Add(new ButtonBase("📁", "$filter$"));

            row.Add(new ButtonBase(NextPageLabel, "$next$"));

            if (EnableSearch) row.Insert(2, new ButtonBase($"🔍 {SearchQuery ?? ""}", "$search$"));
