...
namespace DenpouControls.Examples.Groups;
...
public class LinkReplaceTest : GroupForm
{
    ...
    public override async Task OnMessage(MessageResult e)
    {
        ...
        if (u.Status is ChatMemberStatus.Administrator or ChatMemberStatus.Creator)
        {
        ...
        if (Counter[from] >= 3)
        {
            ...
            await e.Device.Send(
                $"{e.Message.From.FirstName} {e.Message.From.LastName} has been removed from the group");
        }
        else
        {
            ...
            await e.Device.Send(
                $"{e.Message.From.FirstName} {e.Message.From.LastName} has been blocked for 30 seconds");
        }
    }
